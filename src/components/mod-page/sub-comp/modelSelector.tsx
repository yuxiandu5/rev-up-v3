...
export default function ModelSelector({carData, selectedCar, setSelectedCar, disabled}: ModelSelectorProps) {
  ...
  return (
    <div className="flex flex-col w-full max-w-xs">
      ...
      >
        ...
        {selectedCar.make &&
          Object.keys(carData[selectedCar.make]).map((model) => (  
            <option key={model} value={model} className="bg-[var(--bg-dark1)] text-[var(--text1)]">{model}</option>  
          ))
        }
        </select>
