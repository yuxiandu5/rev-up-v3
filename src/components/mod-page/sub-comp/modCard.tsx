    ...
    >
      <div className="flex flex-col gap-3">
        ...
        <div className="flex justify-between items-start">
          <h4 className="text-lg font-semibold text-[var(--text1)]">{mod.name}</h4>
        ...
        <div className="flex flex-col md:grid md:grid-cols-2 gap-4 text-xs">
          {modSpec?.hpGain && modSpec.hpGain !== 0 && (
            <div className="flex justify-between">
              ...
              <span
                className={modSpec.hpGain && modSpec.hpGain > 0 ? "text-green-400" : "text-red-400"}
              >
                {modSpec.hpGain && modSpec.hpGain > 0 ? "+" : ""}
                {modSpec.hpGain} hp
              </span>
          ...
          {modSpec?.nmGain && modSpec.nmGain !== 0 && (
            <div className="flex justify-between">
              ...
              <span
                className={modSpec.nmGain && modSpec.nmGain > 0 ? "text-green-400" : "text-red-400"}
              >
                {modSpec.nmGain && modSpec.nmGain > 0 ? "+" : ""}
                {modSpec.nmGain} Nm
              </span>
          ...
          {modSpec?.zeroToHundredDelta && modSpec.zeroToHundredDelta !== 0 && (
            <div className="flex justify-between">
              ...
              >
                {modSpec.zeroToHundredDelta && modSpec.zeroToHundredDelta > 0 ? "+" : ""}
                {(modSpec.zeroToHundredDelta * 0.1).toFixed(1)}s
              </span>
            </div>
          )}
